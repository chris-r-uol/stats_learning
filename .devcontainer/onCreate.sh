...
#
# This is onCreateCommand, not postCreateCommand, and the difference matters.
# Per the dev container specification, cloud services run onCreateCommand when
# prebuilding, so this work is baked into the prebuilt image. postCreateCommand
# runs "once the dev container has been assigned to a user", which means every
# student would wait for it on every Codespace start, prebuild or not.
#
